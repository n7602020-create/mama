...
import { LayoutGrid, Settings, ListChecks, CalendarRange, Plus, Trash2, Megaphone, BellRing, Image as ImageIcon, Check } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 animate-in fade-in duration-300">
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-1">
                            {event.status === EventStatus.Pending && (
                              <button onClick={() => handleConfirmEvent(event.id)} className="text-emerald-500 p-2 hover:bg-emerald-50 rounded-lg">
        ...
        {activeTab === 'slots' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button onClick={handleAddSlot} className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2">
            ...
            <div className="grid gap-3">
              {localSettings.slots.map(slot => (
                <div key={slot.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex flex-wrap items-center gap-3">
                  ...
                  <button onClick={() => updateSettings({...localSettings, slots: localSettings.slots.filter(s => s.id !== slot.id)})} className="text-red-400 p-2"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </div>
              ...
              {localSettings.fields.map(field => (
                <div key={field.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => updateSettings({...localSettings, fields: localSettings.fields.map(f => f.id === field.id ? {...f, isRequired: !f.isRequired} : f)})} className={`px-3 py-1 rounded-lg text-[10px] font-bold ${field.isRequired ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-500'}`}>
                      {field.isRequired ? 'חובה' : 'רשות'}
                    </button>
                    <button onClick={() => updateSettings({...localSettings, fields: localSettings.fields.map(f => f.id === field.id ? {...f, isPublic: !f.isPublic} : f)})} className={`px-3 py-1 rounded-lg text-[10px] font-bold ${field.isPublic ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500'}`}>
        ...
        {activeTab === 'notices' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold">הנחיות</h2>
              <button onClick={addNotice} className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2"><Plus className="w-4 h-4" /> הוסף</button>
            </div>
            <div className="grid gap-3">
              {localSettings.notices.map(notice => (
                <div key={notice.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex gap-3 items-center">
                  ...
                  <button onClick={() => updateSettings({...localSettings, notices: localSettings.notices.filter(n => n.id !== notice.id)})} className="text-red-400 p-2"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'ads' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button onClick={addAd} className="bg-emerald-600 text-white px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-2"><Plus className="w-4 h-4" /> הוסף</button>
            </div>
            <div className="grid gap-4">
              {localSettings.ads.map(ad => (
                <div key={ad.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4">
                  <input className="w-full bg-white border rounded-lg px-3 py-1.5 font-bold text-sm outline-none" value={ad.title} onChange={e => updateSettings({...localSettings, ads: localSettings.ads.map(a => a.id === ad.id ? {...a, title: e.target.value} : a)})} />
                  <textarea className="w-full bg-white border rounded-lg px-3 py-1.5 text-sm h-16 outline-none resize-none" value={ad.description} onChange={e => updateSettings({...localSettings, ads: localSettings.ads.map(a => a.id === ad.id ? {...a, description: e.target.value} : a)})} />
                  <div className="flex gap-2">
                    <input placeholder="כתובת תמונה" className="flex-grow bg-white border rounded-lg px-3 py-1 text-[10px] outline-none" value={ad.imageUrl || ''} onChange={e => updateSettings({...localSettings, ads: localSettings.ads.map(a => a.id === ad.id ? {...a, imageUrl: e.target.value} : a)})} />
                    <button onClick={() => updateSettings({...localSettings, ads: localSettings.ads.filter(a => a.id !== ad.id)})} className="text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'texts' && (
          <div className="space-y-4">
             <h3 className="font-bold border-b pb-2 text-[10px] uppercase tracking-wider text-slate-400">הגדרות כלליות</h3>
             <div>
               <label className="block text-[10px] font-bold text-slate-400 mb-1">שם מתאמת</label>
             ...
             <div>
               <label className="block text-[10px] font-bold text-slate-400 mb-1">טלפון מתאמת</label>
             ...
             <div>
               <label className="block text-[10px] font-bold text-slate-400 mb-1">הערת מערכת</label>
